...
- `docs/module-ownership.md`、`docs/team-development-workflow.md`、`docs/three-person-development-plan.md`、`docs/mvp-scope.md`、`docs/migration-register.md`：三人开发协作、后端双人分工、分支、迁移、MVP 范围和 owner 规则。
- `docs/api-contract-platform.md`、`docs/database-schema-design.md`、`docs/openapi/platform-api.openapi.yaml`、`docs/openapi/course-module.openapi.yaml`：面向前后端并行开发的接口、数据库与 OpenAPI 契约。
- `docs/requirements-compliance-review.md`：当前代码和设计文档相对需求说明书的符合性审查结论。
...
系统采用 B/S 架构和前后端分离模式。前端使用 Vue 3 + Vite + TypeScript + Element Plus，按学生端、教师端、管理员端组织路由和页面。外部请求统一进入 `edu-gateway`，网关负责路由、JWT 初步鉴权、跨域、traceId、AI 接口限流和 SSE 转发。传统业务由 `edu-biz-service` 负责，数据存储在 MySQL 8.0，并使用 Redis、RabbitMQ、Flyway 和 MyBatis-Plus。AI 能力由 `edu-ai-service` 负责，使用 Spring AI 或 LangChain4j、Qdrant 或 Milvus、Redis 短期任务状态和 SSE 流式输出。服务间同步调用的 DTO 与 Feign Client 统一放在 `edu-feign-api`，避免 AI 或 Gateway 直接依赖 Biz 的 Entity、Mapper 或 Service。
...
`edu-feign-api` 是本次修正后新增的 Maven 子模块，不是第四个应用服务。它负责保存服务间同步调用的 Feign Client 接口、内部 DTO 和版本化上下文契约，例如 AI 服务调用 Biz 的 `/_internal/v1/ai-context/course` 获取授权后的最小课程上下文。该模块只能依赖 `edu-common` 和必要的契约注解，不能放 Biz 数据库实体、Mapper、应用服务实现，也不能承载前端展示 VO。前后端公开 REST API 仍以 `docs/api-contract-platform.md` 和 OpenAPI 文档为准；服务间内部调用以 `edu-feign-api` 中的 Java 契约为准。
...
| 部署、测试与演示数据 | 交付支撑 | 全阶段 | 是 | 否 | 包括本地依赖、Flyway、Docker Compose、测试账号、演示课程、作业、成绩、预警和 AI 示例数据。 |

本节用于明确设计范围和实施优先级，不作为每日开发进度记录。后续开发应尽量遵循本文档设计，若出现架构、接口、数据库、权限或 AI 调用方式等重大变化，应通过版本记录更新设计文档。
...
| 论坛与公告 | 课程论坛、主题、回复、课程公告、系统公告、内容治理 | 学生、教师、管理员 | 输入主题、回复、公告、治理命令；输出列表和状态 | `edu_forum_topic`、`edu_forum_reply`；公告表后续扩展 | `/api/v1/student/courses/{courseId}/forum/topics`、`/api/v1/student/forum/topics/{topicId}/replies`、`/api/v1/teacher/forum/topics/{topicId}/visibility` | 课程成员可见、作者编辑、教师课程治理、管理员全局治理 | 与课程、通知、管理员治理关联 |
...
个人中心（`/account/profile`）是登录用户的自助资料页，顶栏仅保留「个人中心」入口，**退出登录内置于此页**，不再在顶栏单列，避免重复入口。页面由三块构成：

- **个人信息**：头像、显示名称、用户名、用户 ID、角色。头像支持自助更换与移除——先 `POST /api/v1/files`（`purpose=AVATAR`）上传图片得到 `fileId`，再 `PUT /api/v1/auth/me/avatar` 提交 `{fileId, version}` 关联，`fileId=null` 即移除；文件内容接口需鉴权，前端以带 `Authorization` 的 `fetch` 拉取 blob 后经 `createObjectURL` 展示，而非裸 `<img src>`。角色以后端下发的 `roles` 列表转中文合并为**单行**展示，不再并列「当前身份」与「拥有角色」等同义字段；账号来源、资料版本等内部字段不对用户展示。
- **修改密码**：`POST /api/v1/auth/me/password` 提交 `{currentPassword, newPassword}`，服务端 BCrypt 校验当前密码、要求新密码 8～128 位且含字母与数字、且不与旧密码相同；成功后当前令牌仍有效。
- **权限信息**：`permissions` 权限码统一映射为中文（如 `admin:access`→「管理控制台」），未收录的编码回退原码以不丢信息。

设计原则：只呈现真实存在的后端能力，不放置「未开放」占位；信息按「个人信息 / 安全 / 权限」分区，避免要素堆叠。
...
AI 风险解释流程为：教师或学生在预警详情页触发解释，AI 根据授权后的学习记录、作业、成绩和课程进度生成原因说明和改进建议。AI 不改变预警等级或状态，不直接通知学生，也不生成带歧视性或绝对化判断的结论。
...
学生在章节学习页或课程资料页发起问题，前端携带课程、章节、课时和会话 ID 调用 AI SSE 接口。AI 服务先向 Biz 请求授权后的最小上下文，确认学生已选课且资料可见，再检索向量库并调用大模型生成答案。返回内容包含 `meta`、`tool`、`delta`、`citation`、`done` 或 `error` 事件，必须展示引用来源；若无可靠来源，应明确提示"未找到可引用资料"。

后端接入阿里百炼 Spring AI 后，答疑流具备三项增强，前端须对应呈现：

- **多轮对话记忆**：`CourseQaRequest.conversationId` 标识一段连续会话。前端每进入一个课时生成一个会话 ID（`lesson-{lessonId}-{时间戳}`，满足 `^[A-Za-z0-9._:-]*$`、≤64），使同一课时内的追问共享上下文；切换课时即开启新会话。
- **RAG 检索与工具调用可视化**：`tool` 事件（`AiToolEvent`）上报进入模型前/中的授权 RAG 检索与工具调用（如 `course_rag_search`、`list_course_lessons`、`list_course_materials`）进度与命中片段，前端在答疑区展示"正在检索课程资料…命中 N 个片段"，让"引用有据"可见。
- **能力自描述**：`meta` 事件携带 provider/model 与 `ragEnabled`、`toolCallingEnabled`、`conversationMemoryEnabled` 等标志及可用 `tools` 列表，供前端按实际能力渲染。
...
erDiagram
    ...
    edu_course ||--o{ edu_course_notice : publishes
...
| `edu_course_notice`、`edu_system_notice` | 公告表 | 待定 | id | 待定 | scope_status_time | 后续扩展；本次未建表 |
...
本节补充作业、考试、预警和 AI 相关表的字段级设计，用于指导后续 Flyway 迁移和 MyBatis-Plus 实体设计。Long ID 在后端使用 `BIGINT`，对前端 JSON 序列化为字符串；表之间保持逻辑关联，不使用数据库物理外键强绑定，通过业务代码、唯一索引、普通索引和状态流转规则保证一致性。
...
| 认证与用户 | 更新头像 | `PUT /api/v1/auth/me/avatar` | 已登录用户 | fileId、version | CurrentUserVO | 本人，头像文件归属校验 | 当前已实现；fileId=null 移除 |
| 认证与用户 | 修改密码 | `POST /api/v1/auth/me/password` | 已登录用户 | currentPassword、newPassword | 无 | 本人，校验当前密码 | 当前已实现 |
...
| 作业与成绩 | 教师评分发布 | `POST /api/v1/teacher/submissions/{submissionId}/grade` | 教师 | score、comment、publish | GradeVO | 负责课程、版本校验 | 设计接口 |
| 考试与题库 | 创建考试 | `POST /api/v1/teacher/courses/{courseId}/exams` | 教师 | title、startAt、endAt、rules | ExamVO | 负责课程 | 设计接口 |
| 考试与题库 | 题库管理 | `GET/POST /api/v1/teacher/question-banks/{bankId}/questions` | 教师 | type、difficulty、stem、answer | QuestionVO | 题库编辑权 | 设计接口 |
| 考试与题库 | 试卷编排 | `POST /api/v1/teacher/exams/{examId}/papers` | 教师 | questions、scores | ExamPaperVO | 负责课程，考试草稿 | 设计接口 |
| 考试与题库 | 学生考试列表 | `GET /api/v1/student/courses/{courseId}/exams` | 学生 | page、courseId、status | PageResponse | 本人已选课程 | 设计接口 |
| 论坛与公告 | 课程发帖 | `POST /api/v1/student/courses/{courseId}/forum/topics` | 学生 | title、content | ForumTopicVO | 已选课程成员 | 设计接口 |
| 论坛与公告 | 帖子评论 | `POST /api/v1/student/forum/topics/{topicId}/replies` | 学生、教师 | content | ForumReplyVO | 课程成员 | 设计接口 |
...
| 预警 | 学生预警列表 | `GET /api/v1/student/warnings` | 学生 | page、courseId、level | PageResponse | 本人预警 | 设计接口 |
| 预警 | 教师处理预警 | `POST /api/v1/teacher/warnings/{warningId}/handle` | 教师 | action、remark | WarningActionVO | 负责课程 | 设计接口 |
| AI 服务 | 课程答疑流 | `POST /api/v1/ai/courses/{courseId}/qa/stream` | 学生、教师 | lessonId、question | SSE | 继承课程/章节权限 | 设计接口 |
| AI 服务 | 生成评语草稿 | `POST /api/v1/ai/submissions/{submissionId}/comment-draft` | 教师 | rubric、points | AiDraftVO 或 SSE | 负责课程和提交 | 设计接口 |
| AI 服务 | 章节摘要 | `POST /api/v1/ai/lessons/{lessonId}/summary-draft` | 教师 | courseId、lessonIds | AiDraftVO | 负责课程 | 设计接口 |
| AI 服务 | 风险解释 | `POST /api/v1/ai/warnings/{warningId}/explanation` | 学生、教师 | warningId | AiSuggestionVO | 本人或负责课程 | 设计接口 |
| AI 服务 | 组卷建议 | `POST /api/v1/ai/exams/paper-suggestions` | 教师 | examId、constraints | AiPaperSuggestionVO | 负责课程和题库 | 设计接口 |
...
| AI 服务 | 服务状态 | `GET /api/v1/ai/admin/status` | 管理员、超级管理员 | 可选 `X-AI-Api-Key`、`X-AI-Model` | AiServiceStatusVO | AI 管理权限；传密钥即"测试连接" | 当前已实现 |

## 5.4 AI SSE 流式接口设计

AI 流式输出使用 `text/event-stream;charset=UTF-8`。正常顺序为 `meta -> tool* -> delta* -> citation* -> done`，异常顺序为 `meta -> tool* -> delta* -> citation* -> error -> close`。心跳使用 SSE comment，不定义为业务事件。

> 说明：接入百炼 Spring AI 后新增 `tool` 事件，用于在生成前/中上报授权 RAG 检索与工具调用进度；`meta` 事件同时携带 provider/model 与 `ragEnabled`、`toolCallingEnabled`、`conversationMemoryEnabled` 能力标志及可用 `tools` 列表。
...
用途：上报进入模型前/中的授权 RAG 检索与工具调用进度，让"引用有据"对用户可见。数据为 `AiToolEvent`：toolName、status、input、summary、result（`AiCitationVO[]`）。前端在答疑区展示"正在检索课程资料…命中 N 个片段"。

```text
event: tool
id: trace-001:1
data: {"toolName":"course_rag_search","status":"COMPLETED","input":"preflight","summary":"进入模型前完成授权 RAG 检索，命中 5 个片段","result":[{"resourceType":"MATERIAL","resourceId":"82401","title":"课程讲义（全册）","locator":"demo/course-82001/handout.pdf"}]}
```
...
| 个人中心 | `/account/profile` | `/api/v1/auth/me`、`/api/v1/auth/me/avatar`、`/api/v1/auth/me/password`、`/api/v1/files` | 个人信息卡、头像上传、修改密码表单、中文权限标签、退出登录 | 全部已登录用户 | 只可操作本人资料；头像文件归属校验、密码校验当前密码 | 否 | 退出登录内置于此；只呈现真实后端能力，权限码转中文，无冗余内部字段。 |
| AI 设置 | `/admin/ai` | `/api/v1/ai/admin/status`（可带 `X-AI-Api-Key`/`X-AI-Model`） | 服务状态、框架能力（RAG/向量库/工具/对话记忆）、密钥与模型配置、测试连接 | 管理员、超级管理员 | AI 管理权限 | 是 | 密钥仅存浏览器；测试连接用输入值真实探测，失败经 detail 回显。 |
...
共享模块修改规则：`edu-common` 只放技术协议，不放业务实体；`edu-feign-api` 只放跨服务契约 DTO 和 Feign Client，不放数据库实体或实现逻辑；父 POM、公共错误码、JWT claims、Gateway 安全、application 配置、Flyway 和 OpenAPI 需要单独 PR 说明影响。跨模块功能按“契约 -> 提供方 -> 消费方 -> 端到端验证”的顺序合并。

# 附录 A 自查与边界说明

本文档保持“项目设计文档”的定位，重点说明系统如何设计和实现，而不是重复需求分析或记录每日开发进度。文档中对当前仓库已有能力的表述仅用于说明设计依据，例如已有基础认证、网关、课程学习主链和部分课程 API，不把规划中的作业、考试、预警和 AI 能力描述为已经全部实现。
...
| V1.3 | 2026-07-18 | 新增个人中心设计（头像上传/修改密码/中文权限、退出登录内置）；对齐百炼 Spring AI 增强（多轮对话记忆、RAG/工具调用 `tool` 事件可视化、`AiServiceStatusVO` 能力自描述）；接口清单补充 `POST /me/password`、`PUT /me/avatar` 与 `GET /ai/admin/status`；SSE 增加 `tool` 事件；页面映射补充个人中心与 AI 设置 | 让设计与前端追平后端已实现能力，消除冗余展示 | 3.1、3.7、5.3、5.4、6.5 | 第一组 |
