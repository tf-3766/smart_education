...
| `POST` | `/api/v1/teacher/courses/{courseId}/forum/topics` | 教师 | `ForumTopicCreateRequest` | `ForumTopicDetailVO` | 已实现 |
| `GET` | `/api/v1/teacher/forum/topics/{topicId}` | 教师 | 无 | `ForumTopicDetailVO` | 已实现 |
| `GET/POST` | `/api/v1/teacher/forum/topics/{topicId}/replies` | 教师 | `ForumListQuery`、`ForumReplyCreateRequest` | `PageResponse<ForumReplyVO>`、`ForumReplyVO` | 已实现 |
...
`CourseQaRequest` 字段为 `question*: string(1-2000)`、`lessonId?: string`。`LessonSummaryRequest` 字段为 `courseId*: string`。`AiDraftVO` 包含 `requestId`、`draftType`、`businessId`、`content`、`provider`、`model`、`status: DRAFT/FRAMEWORK_ONLY`、`citations: AiCitationVO[]`、`createdAt`。`AiCitationVO` 包含 `resourceType`、`resourceId`、`title`、`locator`。
