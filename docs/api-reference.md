...
协作教师由课程负责人发出邀请，新关系为 `PENDING`，只有受邀教师接受后变为 `ACTIVE` 并获得课程编辑权限；拒绝后关系逻辑删除，负责人可以重新邀请。`CourseTeacherVO.status` 固定为 `PENDING/ACTIVE`。课程模板来自已有课程的可复用基本信息，教师目录只返回已启用教师，学期窗口由管理员统一维护。
### 3.4 教师课程内容管理
...
SSE 事件固定为 `meta`、`tool`、`delta`、`citation`、`done`、`error`，每个事件的 JSON 数据为 `AiStreamEvent`：`type`、`requestId`、`data`、`timestamp`；同一次请求的全部事件共用一个 `requestId`。`meta.data` 会说明模型、向量库、会话编号和工具调用是否可用，`tool.data` 会说明课程知识检索是完成还是跳过。`error.data` 为 `{code,message}`，Biz 返回的 401/403/404/409 会保留对应统一错误码，模型或下游不可用才返回 `AI_SERVICE_UNAVAILABLE`。
...
当前工具调用使用 Spring AI `@Tool` 暴露“列出授权课时/资料”两个进程内工具，模型可在多轮课程问答中调用；尚未部署外部 MCP Server/Client，不能把内部工具调用误写成 MCP 已完成。AI 只能返回回答、建议、草稿和引用，正式业务数据必须由 Biz 服务在人工确认后写入。
