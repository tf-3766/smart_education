...
当前公开接口已覆盖登录注册、教师审核、超级管理员授权、文件与头像、课程及协作教师邀请、学期选课窗口、课程内容、选课、学习进度、课程审核、作业、成绩、论坛治理、预警、考试题库与答题、公告、课程分类和管理统计。AI 已提供课程 RAG 问答、多轮会话、课程工具调用、知识库同步与状态、课时摘要、批改评语、风险解释和组卷建议；未配置模型或向量库时明确降级，不生成伪答案。
...
| `POST` | `/api/v1/auth/register-with-avatar` | 匿名 | `multipart/form-data`：`data` 为 `RegisterRequest` JSON、`avatar?` 为头像文件 | `RegistrationVO`；头像可选 |
| `GET` | `/api/v1/auth/me` | 已登录 | 无 | `CurrentUserVO` |
| `PUT` | `/api/v1/auth/me/avatar` | 已登录 | `UpdateAvatarRequest` | `CurrentUserVO` |
| `POST` | `/api/v1/auth/me/password` | 已登录 | `ChangePasswordRequest` | `null` |
...
公开注册的 `role` 只接受 `STUDENT` 或 `TEACHER`。注册页可选头像；携带头像时调用 `register-with-avatar`，头像仅接受 JPEG、PNG、WebP、GIF，最大尺寸遵循文件上传配置。用户名保存前会去除首尾空白并转为小写。学生账号以 `ENABLED` 创建，`RegistrationVO.login` 返回登录令牌；教师账号以 `PENDING` 创建，`login=null`，必须由超级管理员审核后才能登录。普通注册密码和修改后的新密码必须为 8～128 个字符并同时包含字母和数字；修改密码还会校验当前密码，成功后现有 JWT 继续有效；Bootstrap 演示账号的固定密码不经过注册接口。本地 Bootstrap 账号为 `student/123456`、`teacher/t123456`、`teacher2/t123456`、`admin/admin123`，数据库仅保存 BCrypt 哈希；这些固定凭据只用于本地开发和验收，生产环境不得保留。
...
| `GET` | `/api/v1/files/{fileId}/preview?page={page}` | 同上 | `page` 从 0 开始，仅 PPTX 需要 | 图片流；PPTX 按页渲染 PNG，页数见 `X-Preview-Page-Count` |
...
| 方法 | 路径 | 请求 | 响应 |
|---|---|---|---|
| `GET` | `/api/v1/teacher/courses/templates` | 无 | `List<CourseTemplateVO>` |
| `GET` | `/api/v1/teacher/courses/term-windows` | 无 | `List<TermEnrollmentWindowVO>` |
| `GET` | `/api/v1/teacher/courses/teacher-directory` | 无 | `List<TeacherOptionVO>` |
| `GET` | `/api/v1/teacher/courses/collab-invitations` | 无 | `List<CollabInvitationVO>` |
| `POST` | `/api/v1/teacher/courses/collab-invitations/{courseId}/accept` | 无 | `void` |
| `POST` | `/api/v1/teacher/courses/collab-invitations/{courseId}/reject` | 无 | `void` |
| `POST` | `/api/v1/teacher/courses` | `CreateCourseRequest` | `CourseDetailVO`，201 |
| `GET` | `/api/v1/teacher/courses` | `CourseListQuery`；正式业务传 `formalOnly=true` | `PageResponse<TeacherCourseListItemVO>` |
...
| `DELETE` | `/api/v1/teacher/courses/{courseId}` | 路径参数 | `void`；仅负责人可删除未提交/已驳回草稿 |
...
协作教师由课程负责人发出邀请，新关系为 `PENDING`，只有受邀教师接受后变为 `ACTIVE` 并获得课程编辑权限；拒绝后关系逻辑删除，负责人可以重新邀请。`CourseTeacherVO.status` 固定为 `PENDING/ACTIVE`。课程模板来自已有课程的可复用基本信息，教师目录只返回已启用教师，学期窗口由管理员统一维护。`CourseListQuery.formalOnly=true` 固定筛选 `reviewStatus=APPROVED` 且课程状态为 `PUBLISHED/ONGOING/FINISHED`，供作业、考试、预警和课程互动等正式业务共用。
### 3.4 教师课程内容管理
...
| `ChangePasswordRequest` | `currentPassword*: string(1-128)`，`newPassword*: string(8-128，必须同时包含字母和数字)` |
...
| `CourseTeacherVO` | `relationId`，`courseId`，`teacherId`，`teacherName`，`role: CodeLabel`，`status: PENDING/ACTIVE`，`version` |
| `CollabInvitationVO` | `courseId`，`courseName`，`courseCode`，`ownerTeacherName`，`status: PENDING/ACTIVE`，`invitedAt` |
| `CourseTemplateVO` | `templateId`，`name`，`summary?`，`term?`，`department?`，`credit?` |
| `TeacherOptionVO` | `teacherId`，`displayName`，`username` |
| `TermEnrollmentWindowVO` | `windowId`，`term`，`enrollmentOpenAt?`，`enrollmentCloseAt?`，`version` |
...
| `GET` | `/api/v1/teacher/assignments/{assignmentId}/submission-roster` | 教师 | 无 | `List<TeacherSubmissionRosterVO>`，包含已提交与未提交学生 | 已实现 |
...
作业状态为 DRAFT/PUBLISHED/CLOSED；提交状态为 DRAFT/SUBMITTED/RETURNED/GRADED；成绩发布状态为 DRAFT/PUBLISHED。作业完成方式为 MIXED/TEXT/CODE/QUIZ，题目支持 SINGLE_CHOICE/MULTI_CHOICE/TRUE_FALSE/FILL_BLANK/SHORT_ANSWER。正式提交必须至少包含在线回答、结构化题目答案或附件。学生读取题目作业时 correctAnswers 固定为空，教师端保留参考答案。已发布作业一旦存在提交，lessonId/responseMode/questions/maxScore/openAt/dueAt 即冻结。
...
| `PATCH` | `/api/v1/teacher/forum/topics/{topicId}/pin` | 教师 | `ForumPinRequest` | `ForumTopicListItemVO` | 已实现，课程编辑者可置顶或取消置顶 |
...
| `GET` | `/api/v1/admin/forum/topics` | 管理员 | `ForumListQuery` | `PageResponse<ForumTopicListItemVO>` | 已实现 |
| `GET` | `/api/v1/admin/forum/replies` | 管理员 | `ForumListQuery` | `PageResponse<ForumReplyVO>` | 已实现 |
...
| `GET` | `/api/v1/admin/term-enrollment-windows` | 管理员 | 无 / `List<TermEnrollmentWindowVO>` | 已实现 |
| `PUT` | `/api/v1/admin/term-enrollment-windows` | 管理员 | `UpsertTermEnrollmentWindowRequest` / `TermEnrollmentWindowVO` | 已实现 |
...
| `UpsertTermEnrollmentWindowRequest` | `term*: string(格式 YYYY 春季/秋季)`，`enrollmentOpenAt?`，`enrollmentCloseAt?`；两者都存在时截止时间必须晚于开始时间 |
...
| 方法 | 路径 | 角色 | 请求 / 响应 | 状态 |
|---|---|---|---|---|
| `GET` | `/api/v1/notifications` | 已登录 | `NotificationListQuery` / `PageResponse<NotificationVO>` | 已实现 |
| `GET` | `/api/v1/notifications/unread-count` | 已登录 | 无 / `long` | 已实现 |
| `POST` | `/api/v1/notifications/{notificationId}/read` | 已登录 | 无 / `void` | 已实现，仅操作本人通知 |
| `POST` | `/api/v1/notifications/read-all` | 已登录 | 无 / `void` | 已实现 |
| `POST` | `/api/v1/notifications/{notificationId}/archive` | 已登录 | 无 / `void` | 已实现，仅操作本人通知 |
| `GET` | `/api/v1/notifications/preferences` | 已登录 | 无 / `NotificationPreferencesVO` | 已实现 |
| `PUT` | `/api/v1/notifications/preferences` | 已登录 | `UpdateNotificationPreferencesRequest` / `NotificationPreferencesVO` | 已实现 |
| `GET` | `/api/v1/notifications/stream` | 已登录 | 无 / `text/event-stream` | 已实现，事件用于提示前端刷新通知数据 |

`NotificationListQuery` 支持 `page`、`size`、`category?: COURSE/ASSIGNMENT/EXAM/WARNING/SYSTEM` 和 `unread?: boolean`。`NotificationVO` 包含 `notificationId`、`title`、`content`、`category`、`categoryLabel`、`status`、`sourceType`、相关业务 ID、`createdAt`、`read`、`readAt?`。偏好请求与响应使用 `enabledCategories` 集合；关闭某分类只影响后续投递，不越权读取其他用户通知。
### 4.5 AI 接口

| 方法 | 路径 | 角色 | 请求 / 响应 | 状态 |
|---|---|---|---|---|
| `GET` | `/api/v1/ai/capabilities?courseId={courseId?}` | 已登录（学生/教师/管理员） | 无 / `AiCapabilityVO[]` | 已实现；前端入口、提示词和后端工具共用同一能力注册表 |
| `POST` | `/api/v1/ai/assistant/stream` | 已登录（学生/教师/管理员） | `AssistantChatRequest` / `text/event-stream` | 已实现；有课程上下文时复用 RAG 与工具调用 |
| POST | /api/v1/ai/courses/{courseId}/qa/stream | 学生/教师 | CourseQaRequest / 	ext/event-stream | 已实现，使用授权课时正文 |
| `GET` | `/api/v1/ai/courses/{courseId}/knowledge-base/status` | 教师 | 无 / `AiKnowledgeBaseStatusVO` | 已实现，检查授权后返回课程知识库状态 |
| `POST` | `/api/v1/ai/courses/{courseId}/knowledge-base/sync` | 教师 | 无 / `AiKnowledgeBaseStatusVO` | 已实现，按授权课时正文及已发布资料元数据切分并写入 Qdrant |
| `POST` | `/api/v1/ai/lessons/{lessonId}/summary-draft` | 教师 | `LessonSummaryRequest` / `AiDraftVO` | 已实现，仅返回草稿 |
| `POST` | `/api/v1/ai/admin/governance-review-draft` | 管理员/超级管理员 | `AdminGovernanceDraftRequest` / `AdminGovernanceDraftVO` | 已实现；教师与课程合计最多 50 项，只返回逐项预审草稿，不执行审批或发布；教师注册预审仅超级管理员可用，普通管理员只能检查课程 |
...
| AI DTO / VO | JSON 字段 |
|---|---|
| `AssistantChatRequest` | `question*: string(1-2000)`，`courseId?: string`，`lessonId?: string`，`pagePath?: string(<=240)`，`pageTitle?: string(<=120)`，`conversationId?: string(<=64，仅 A-Z/a-z/0-9/._:-)` |
| CourseQaRequest | question*: string(1-2000)，lessonId?: string，conversationId?: string(<=64，仅 A-Z/a-z/0-9/._:-)；同一用户、课程和会话编号共享最近 16 条消息 |
...
| `AdminGovernanceDraftRequest` | `teacherUserIds?: string[]`，`courseIds?: string[]`，`criteria?: string(<=2000)`；两类 ID 合计 1-50 项 |
| `AdminGovernanceDraftVO` | `requestId`，`status: DRAFT/FRAMEWORK_ONLY`，`totalCount`，`successCount`，`failureCount`，`reviewCount`，`teacherReviews[]`，`courseCompliance[]`，`createdAt`；教师项包含 `targetVersion`、账号、姓名、注册时间、建议、置信度、风险码、原因和证据；课程项包含 `targetVersion`、简介、分类、学期、院系、学分、课程/选课时间窗、逐项失败、课时/资料数量、准备度、问题码、整改原因和证据 |
...
| `AiKnowledgeBaseStatusVO` | `courseId`，`vectorStoreConfigured: boolean`，`indexedChunks`，`lastSyncedAt?`；同步计数和时间为当前 AI 进程记录 |
| `AiCapabilityVO` | `capabilityId`，`name`，`description`，`roles[]`，`mode: ANSWER/DRAFT/ACTION`，`riskLevel`，`requiredContext[]`，`confirmationPolicy`，`deepLinkTemplate?`，`enabled`，`unavailableReason?` |
| `AiStreamEvent` | `type`，`requestId`，`data`，`timestamp` |

SSE 事件固定为 `meta`、`capability`、`tool`、`action`、`delta`、`citation`、`done`、`error`，每个事件的 JSON 数据为 `AiStreamEvent`：`type`、`requestId`、`data`、`timestamp`；同一次请求的全部事件共用一个 `requestId`。`capability.data` 是与能力查询接口相同的角色能力目录，`action.data` 是草稿或待确认动作卡。`meta.data` 会说明模型、向量库、会话编号和工具调用是否可用，`tool.data` 会说明检索与工具调用进度。`error.data` 为 `{code,message}`，Biz 返回的 401/403/404/409 会保留对应统一错误码，模型或下游不可用才返回 `AI_SERVICE_UNAVAILABLE`。

AI 服务基于 Spring AI 1.1.8，通过 OpenAI 兼容协议接入阿里云百炼。聊天默认关闭；设置 `AI_CHAT_PROVIDER=openai` 和 `DASHSCOPE_API_KEY` 后启用 `ChatClient`。RAG 还需同时设置 `AI_EMBEDDING_PROVIDER=openai`、`AI_VECTOR_STORE_TYPE=qdrant` 及 Qdrant 地址。向量库未配置、未命中或查询暂时失败时，问答自动回退到 Biz 已授权的课时正文。课程正文中的指令按不可信参考数据处理。


#### 知识库同步说明

教师不需要进入 Qdrant 控制台建集合或手工上传文件。正确流程是：在课程内容页上传并发布 PDF、Word、PPT、图片等资料，再点击“同步知识库”。业务服务先校验教师权限并读取受保护文件：PDFBox 抽取 PDF 可选择文本，Apache Tika/POI 抽取 Word、PPT 与常见文档正文；图片或扫描 PDF 在本机可用 Tesseract 时执行 OCR。抽取结果连同课时正文按固定长度和重叠窗口分片，AI 服务生成向量后写入 Qdrant，并保留课程、课时、资料和片段定位元数据。问答时只检索当前用户有权访问的课程上下文。Qdrant 是平台内部向量存储，不是教师操作入口；未安装 Tesseract 时普通 Office/PDF 仍可抽取，但纯图片与扫描件会明确标记为 OCR 不可用。
当前工具调用使用 Spring AI `@Tool` 暴露角色化只读查询、课程 RAG、教师跨课程草稿创作和管理员/教师待确认动作。教师可从任意页面先查询本人课程，调用授权课程检查读取章节、课时与资料，再在一个明确命令中创建题库、作业、考试或公告 AI 草稿。学生不会注入任何草稿或动作工具。尚未部署外部 MCP Server/Client，不能把内部工具调用误写成 MCP 已完成。正式业务数据必须由 Biz 服务在人工确认后写入。

完整角色能力和前后端对应关系见 [AI 角色能力矩阵](./ai-role-capability-matrix.md)。

### 4.6 作业、成绩、论坛与预警的数据格式

本节是 Backend A 已实现接口的字段契约。`datetime` 为 RFC 3339 字符串，`decimal` 为 JSON 数字，`?` 表示可为 `null`。

| 请求体类型 | JSON 字段 |
|---|---|
| `AssignmentCreateRequest` | `lessonId?`，`title*`，`description?`，`responseMode?: MIXED/TEXT/CODE/QUIZ`，`questions?: AssignmentQuestionRequest[]`，`maxScore*`，`openAt?`，`dueAt*`，`attachments?` |
| `AssignmentUpdateRequest` | 与创建请求相同，并增加 `version*` |
| `AssignmentQuestionRequest` | `questionId*`，`questionType*`，`stem*`，`options: string[]`，`score*`，`correctAnswers: string[]`；选择和判断答案为选项下标字符串，填空为参考答案文本 |
...
| `SubmissionSaveRequest` | `content?`，`answers?: Record<string,string[]>`，`fileId?`，`fileKey?`，`fileUrl?`，`version?`；保存草稿时可为空 |
| `SubmissionSubmitRequest` | 字段同草稿；`content`、`answers` 或附件引用必须至少有一项；托管与外部附件模式不可混用 |
...
| `WarningHandleRequest` | `action*: HANDLED | IGNORED`，`remark?: string(<=4000)`，`version*: integer` |
...
| 响应 `data` 类型 | 字段 |
|---|---|
| `AssignmentDetailVO` | `assignmentId`，`courseId`，`lessonId?`，`title`，`description?`，`responseMode`，`questions: AssignmentQuestionRequest[]`，`maxScore`，`assignmentStatus: CodeLabel`，`availabilityStatus: CodeLabel`，`openAt?`，`dueAt`，`publishedAt?`，`attachments: AssignmentAttachmentVO[]`，`version`；学生端题目中的 `correctAnswers` 为空 |
...
| `SubmissionDetailVO` | `submissionId`，`assignmentId`，`courseId`，`studentId`，`attemptNo`，`content?`，`answers: Record<string,string[]>`，`fileId?`，`fileKey?`，`fileUrl?`，`submissionStatus: CodeLabel`，`submittedAt?`，`score?`，`teacherComment?`，`aiCommentDraftId?`，`gradedBy?`，`gradedAt?`，`publishedAt?`，`version` |
| `TeacherSubmissionGradeVO` | `submissionId`，`assignmentId`，`courseId`，`studentId`，`studentName?`，`submissionStatus: CodeLabel`，`submittedAt?`，`content?`，`answers: Record<string,string[]>`，`fileId?`，`fileKey?`，`fileUrl?`，`score?`，`maxScore`，`teacherComment?`，`aiCommentDraftId?`，`gradedBy?`，`gradedAt?`，`gradeId?`，`gradeStatus?: CodeLabel`，`publishedAt?`，`version`，`gradeVersion?` |
...
| 方法 | 路径 | 调用方 | 提供方 | 状态 |
|---|---|---|---|---|
| `POST` | `/_internal/v1/ai-context/assistant` | `edu-ai-service` | `edu-biz-service` | 已实现，按学生/教师/管理员/超级管理员返回角色化只读工具快照 |
...
| 内部类型 | JSON 字段 |
|---|---|
| `AiAssistantContextRequest` | `userId*`，`roleCode*`，`traceId?`，`domains[]`；全局助手按问题只请求 `WINDOWS/WARNINGS/ASSIGNMENTS/EXAMS/PROGRESS/GRADES/SUBMISSIONS/ANNOUNCEMENTS/NOTIFICATIONS/FORUM/METRICS/USERS` 中需要的事实域，内部批处理可传 `ALL` |
...
| `AiCourseContextResponse` | `courseId`，`courseCode`，`courseName`，`courseStatus`，`reviewStatus`，`ownerTeacherId`，`teacherMember: boolean`，`enrolled: boolean`，`chapters: AiChapterRef[]`，`lessons: AiLessonRef[]`，`materials: AiMaterialRef[]`；还包含课程简介、分类、学期、院系、学分、课程/选课时间窗和版本 |
| `AiChapterRef` | `chapterId`，`title`，`description?`，`sortOrder`，`status` |
...
| 400 | `CURRENT_PASSWORD_INCORRECT` | 提示当前密码不正确，不清理登录态 |
...
| 409 | `PASSWORD_UNCHANGED` | 提示新密码不能与当前密码相同 |
